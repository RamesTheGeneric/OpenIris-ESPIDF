...
void TXStream::send_jpeg_frame(const uint8_t *jpeg, size_t len)
{
    ...
    while (offset < len) {
        ...
        *custom_hdr++ = chunk_len & 0xFF;           // Length low byte
        ...
        // Add FCS calculation if needed (optional for promiscuous mode)
        uint32_t fcs = calculate_fcs(buffer, frame_len);
        memcpy(custom_hdr, &fcs, 4);
        frame_len += 4;
        ...
        offset += chunk_len;
        vTaskDelay(pdMS_TO_TICKS(0.05)); // Small delay to not overload the poor thing
    }
    printf("Sent Frame %d\n", frame_id);
...
// Simple FCS calculation (CRC32), idk if this actually works ngl
uint32_t TXStream::calculate_fcs(const uint8_t *data, size_t len)
{
